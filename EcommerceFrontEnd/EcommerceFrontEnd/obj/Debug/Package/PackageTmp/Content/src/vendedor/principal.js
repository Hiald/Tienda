                ...
                $.each(data.rDato, function (index, value) {
                    if (index === 0) {
                        ...
                        $('#divPedido').html(
                            ...
                            + "<button class='btn btn-info' type='button' onclick='fn_VerDetalle(" + value.ventaid + ", " + value.envioid + ")' >Ver Detalle</button>"
                            + '</div>'
                            + '</div>'
                            + '</div>'
                        );
...
function fn_VerDetalle(ventaid, envioid) {
...
function fn_NotificarVenta() {
    $.ajax({
        type: "POST",
        url: gRoute + "venta/NotificarVenta",
        async: false,
        success: function (data) {
            console.log(data);
            /* $.each(data.rDatoV, function (index, value) {
...
// se debe crear el archivo vacio primero
async function registerSW() {
    await navigator.serviceWorker.register(RouteServer + '/Content/js/service-worker.js');
}

function getSW() {
    return navigator.serviceWorker.getRegistration(RouteServer + '/Content/js/service-worker.js');
}
